...
class Pisado {
	...
	public static function findByCentro($centro) {
		$db = new DB(SQL_DB_PISADO);
		$db->run('SELECT A.* FROM pisado A INNER JOIN delegados.titulaciones B ON A.id_titulacion = B.id_titulacion WHERE B.id_centro = ? AND B.id_group = 0 ORDER BY A.date DESC', array($centro));
		$data = $db->data();
