...
class Group {
	...
	public static function findById($id,$archive=false) {
		...
		$db->run('SELECT A.*, id_titulacion, curso FROM "group" A LEFT JOIN pisado B ON A.id = B.id_group WHERE A.id=? GROUP BY B.id_group ORDER BY A.date', array($id));
	...
	public static function findByNia($nia, $all = false) {
		$db = new DB(SQL_DB_PISADO);
		$db->run('SELECT A.*, MAX(id_titulacion), MAX(curso) FROM "group" A LEFT JOIN pisado B ON A.id = B.id_group WHERE B.nia=? GROUP BY B.id_group ORDER BY A.date DESC', array($nia));
		$data = $db->data();
	...
	public static function findByIdTitulacion($id_titulacion, $all = false, $archive = false) {
		$db = new DB(SQL_DB_PISADO);
		$db->run('SELECT A.*, id_titulacion, curso FROM "group" A LEFT JOIN pisado B ON A.id = B.id_group WHERE B.id_titulacion=? GROUP BY B.id_group ORDER BY A.date DESC', array($id_titulacion));
		$data = $db->data();
	...
	public static function findByCurso($curso, $id_titulacion, $all = false, $archive = false) {
		$db = new DB(SQL_DB_PISADO);
		$db->run('SELECT A.*, id_titulacion, curso FROM "group" A LEFT JOIN pisado B ON A.id = B.id_group WHERE B.curso=? AND B.id_titulacion=? GROUP BY B.id_group ORDER BY A.date DESC', array($curso,$id_titulacion));
		$data = $db->data();
	...
	public static function findByCentro($centro, $all = false, $archive = false) {
		$db = new DB(SQL_DB_PISADO);
		$db->run('SELECT A.*, MAX(B.id_titulacion), MAX(B.curso) FROM "group" A LEFT JOIN pisado B ON A.id = B.id_group GROUP BY B.id_group ORDER BY A.date DESC');
		$data = $db->data();
